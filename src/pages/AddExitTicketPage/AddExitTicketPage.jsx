...
export default function AddExitTicketPage({setActiveModule, setPopulatedModules, populatedModules}) {
    const [index, setIndex] = useState([])
    const[activeExitTicket, setActiveExitTicket] = useState(null)
    
    const indexExitTickets = index.map(ExitTicket => <ExitTicketer ExitTicket={ExitTicket} key={ExitTicket._id} addExitTicket={addExitTicket} viewExitTicket={viewExitTicket}/>)
        

    function addExitTicket(ExitTicket){
        const populatedModulesCopy = populatedModules
        populatedModulesCopy.splice(0, 1, ExitTicket)
    ...
    function handleUpdate(updatedExitTicket){
        ...
        const idx = indexCopy.findIndex(ExitTicket => ExitTicket._id === updatedExitTicket._id)
    ...
    function handleRemove(deletedExitTicketId){
        const indexCopy = [...index]
        const idx = indexCopy.findIndex(ExitTicket => ExitTicket._id === deletedExitTicketId)
    ...
    async function viewExitTicket(ExitTicket){
        ...
        setActiveExitTicket(<ViewExitTicketForm ExitTicket={viewExitTicket} handleUpdate={handleUpdate} handleRemove={handleRemove}/>)
    }
