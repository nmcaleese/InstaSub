...
export default function SubPlanIndexPage() {
  ...
  const indexSubPlans = index.map((SubPlan) => (
    <SubPlaner
      SubPlan={SubPlan}
      key={SubPlan._id}
      deleteSubPlan={deleteSubPlan}
      viewSubPlan={viewSubPlan}
    />
  ));
  ...
  async function viewSubPlan(SubPlan) {
    ...
    setActiveSubPlan(
      <ViewSubPlanForm SubPlan={viewSubPlan} deleteSubPlan={deleteSubPlan} />
    );
  }
  ...
  function handleRemove(deletedSubPlanId) {
    ...
    const idx = indexCopy.findIndex(
      (SubPlan) => SubPlan._id === deletedSubPlanId
    );
