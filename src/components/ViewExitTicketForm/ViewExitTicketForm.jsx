...
export default function ViewExitTicketForm({ExitTicket, handleUpdate, handleRemove}) {
  ...
  async function handleDelete(){
    try {
      const deletedExitTicketId = await ExitTicketAPI.deleteExitTicket(ExitTicket)
              ...
              <Card.Body className='p-3'>
                ...
                <Form autoComplete='off' onSubmit={handleSubmit}>
                      <Form.Group className="mb-3" controlId={ticket.title}>
                          <Form.Control type='text' placeholder='What do you want to call this Exit Ticket?' name='title' onChange={handleChange} value={ticket.title} required />
                      </Form.Group>
                      <Form.Group className="mb-3" controlId={ticket.question}>
                        <Form.Control as="textarea" rows={2} placeholder='What question should students respond to?' name='question' onChange={handleChange} value={ticket.question} required />
                      </Form.Group>
                  <br />
